...
_ffxi.i[0x56] = function(id, data)
    ...
    local quest_data_updated = false
    ...
    if key and style then
        ffxi.quests[key][style] = bit_flags(p['Quest Flags'])
        quest_data_updated = true
    end
    
    -- Trigger quest update callbacks
    if quest_data_updated and ffxi.quest_update_callbacks then
        for _, callback in ipairs(ffxi.quest_update_callbacks) do
            callback()
        end
    end
end
...
do
    ...
    for id in L{0x00B,0x00A,0x01C,0x037,0x056,0x057,0x061,0x062,0xC8,0xDF,0x076,0x111,0x113,0x118}:it() do
